...
describe('InitCommand', () => {
  ...
  describe('execute with --tools flag', () => {
    it('should create OpenSpec directory structure', async () => {
      ...
      const openspecPath = path.join(testDir, 'flow-studio');
    ...
    it('should create config.yaml with default schema', async () => {
      ...
      const configPath = path.join(testDir, 'flow-studio', 'config.yaml');
    ...
    it('should create skills in Cursor skills directory', async () => {
      ...
      const skillFile = path.join(testDir, '.cursor', 'skills', 'flow-studio-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    });
    ...
    it('should create skills in Windsurf skills directory', async () => {
      ...
      const skillFile = path.join(testDir, '.windsurf', 'skills', 'flow-studio-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    });
    ...
    it('should support Kimi CLI as an adapterless skills-only tool', async () => {
      ...
      const skillFile = path.join(testDir, '.kimi', 'skills', 'flow-studio-explore', 'SKILL.md');
      expect(await fileExists(skillFile)).toBe(true);
    ...
    it('should create skills for multiple tools at once', async () => {
      ...
      const claudeSkill = path.join(testDir, '.claude', 'skills', 'flow-studio-explore', 'SKILL.md');
      const cursorSkill = path.join(testDir, '.cursor', 'skills', 'flow-studio-explore', 'SKILL.md');
    ...
    it('should select all tools with --tools all option', async () => {
      ...
      const claudeSkill = path.join(testDir, '.claude', 'skills', 'flow-studio-explore', 'SKILL.md');
      const cursorSkill = path.join(testDir, '.cursor', 'skills', 'flow-studio-explore', 'SKILL.md');
      const windsurfSkill = path.join(testDir, '.windsurf', 'skills', 'flow-studio-explore', 'SKILL.md');
    ...
    it('should handle non-existent target directory', async () => {
      ...
      const openspecPath = path.join(newDir, 'flow-studio');
      expect(await directoryExists(openspecPath)).toBe(true);
    });
  ...
  describe('skill content validation', () => {
    it('should generate valid SKILL.md with YAML frontmatter', async () => {
      ...
      expect(content).toContain('name: flow-studio-explore');
      expect(content).toContain('description:');
    ...
    it('should include propose skill instructions', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'flow-studio-propose', 'SKILL.md');
      const content = await fs.readFile(skillFile, 'utf-8');

      expect(content).toContain('name: flow-studio-propose');
    });
    ...
    it('should include apply-change skill instructions', async () => {
      ...
      const skillFile = path.join(testDir, '.claude', 'skills', 'flow-studio-apply-change', 'SKILL.md');
      const content = await fs.readFile(skillFile, 'utf-8');

      expect(content).toContain('name: flow-studio-apply-change');
    });
  ...
  describe('command generation', () => {
    it('should generate Claude Code commands with correct format', async () => {
      ...
      const cmdFile = path.join(testDir, '.claude', 'commands', 'fwst', 'explore.md');
    ...
    it('should generate Cursor commands with correct format', async () => {
      ...
      const cmdFile = path.join(testDir, '.cursor', 'commands', 'fwst-explore.md');
      expect(await fileExists(cmdFile)).toBe(true);
  ...
  describe('tool-specific adapters', () => {
    it('should generate Gemini CLI commands as TOML files', async () => {
      ...
      const cmdFile = path.join(testDir, '.gemini', 'commands', 'fwst', 'explore.toml');
      expect(await fileExists(cmdFile)).toBe(true);
    ...
    it('should generate Windsurf commands', async () => {
      ...
      const cmdFile = path.join(testDir, '.windsurf', 'workflows', 'fwst-explore.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
    ...
    it('should generate Continue prompt files', async () => {
      ...
      const cmdFile = path.join(testDir, '.continue', 'prompts', 'fwst-explore.prompt');
      expect(await fileExists(cmdFile)).toBe(true);

      const content = await fs.readFile(cmdFile, 'utf-8');
      expect(content).toContain('name: fwst-explore');
    ...
    it('should generate Cline workflow files', async () => {
      ...
      const cmdFile = path.join(testDir, '.clinerules', 'workflows', 'fwst-explore.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
    ...
    it('should generate GitHub Copilot prompt files', async () => {
      ...
      const cmdFile = path.join(testDir, '.github', 'prompts', 'fwst-explore.prompt.md');
      expect(await fileExists(cmdFile)).toBe(true);
    });
  });
});
...
describe('InitCommand - profile and detection features', () => {
  ...
  it('should use --profile flag to override global config', async () => {
    ...
    const proposeSkill = path.join(testDir, '.claude', 'skills', 'flow-studio-propose', 'SKILL.md');
    expect(await fileExists(proposeSkill)).toBe(true);

    // Non-core skills (from the custom profile) should NOT be created
    const newChangeSkill = path.join(testDir, '.claude', 'skills', 'flow-studio-new-change', 'SKILL.md');
  ...
  it('should auto-cleanup legacy artifacts in non-interactive mode without --force', async () => {
    ...
    await fs.writeFile(path.join(legacyDir, 'fwst-propose.md'), 'legacy content');
    ...
    expect(await fileExists(path.join(legacyDir, 'fwst-propose.md'))).toBe(false);
  ...
  it('should preselect configured tools but not directory-detected tools in extend mode', async () => {
    // Simulate existing Flow Studio project (extend mode).
    await fs.mkdir(path.join(testDir, 'flow-studio'), { recursive: true });

    // Configured with Flow Studio
    ...
    // Directory detected only (not configured with Flow Studio)
  ...
  it('should respect custom profile from global config', async () => {
    ...
    const exploreSkill = path.join(testDir, '.claude', 'skills', 'flow-studio-explore', 'SKILL.md');
    const newChangeSkill = path.join(testDir, '.claude', 'skills', 'flow-studio-new-change', 'SKILL.md');
  ...
  it('should migrate commands-only extend mode to custom profile without injecting propose', async () => {
    await fs.mkdir(path.join(testDir, 'flow-studio'), { recursive: true });
    await fs.mkdir(path.join(testDir, '.claude', 'commands', 'fwst'), { recursive: true });
    await fs.writeFile(path.join(testDir, '.claude', 'commands', 'fwst', 'explore.md'), '# explore\n');
    ...
    const exploreCommand = path.join(testDir, '.claude', 'commands', 'fwst', 'explore.md');
    const proposeCommand = path.join(testDir, '.claude', 'commands', 'fwst', 'propose.md');

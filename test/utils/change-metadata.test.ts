...
describe('writeChangeMetadata', () => {
  ...
  beforeEach(async () => {
    ...
    changeDir = path.join(testDir, 'flow-studio', 'changes', 'test-change');
  ...
  it('should write valid YAML metadata file', async () => {
    ...
    const metaPath = path.join(changeDir, '.flow-studio.yaml');
...
describe('resolveSchemaForChange', () => {
  ...
  it('should use project config schema when no metadata exists', async () => {
    ...
    const configDir = path.join(testDir, 'flow-studio');

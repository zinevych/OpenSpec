...
/**
 * Generates Zsh completion scripts for the flow-studio CLI.
 * Follows Zsh completion system conventions using the _flow_studio function.
 */
export class ZshGenerator implements CompletionGenerator {
  ...
  generate(commands: CommandDefinition[]): string {
    ...
    for (const cmd of commands) {
      ...
      commandCaseLines.push(`          _flow_studio_${this.sanitizeFunctionName(cmd.name)}`);
    ...
    return `#compdef flow-studio

# Zsh completion script for flow-studio CLI
...
${commandList}
  ...
  case $state in
    command)
      _describe "flow-studio command" commands
      ;;
    args)
      case $words[1] in
${commandCases}
...
${commandFunctions}
${helpers}
compdef _flow_studio flow-studio
`;
  }
  ...
  private generateCommandFunction(cmd: CommandDefinition): string[] {
    const funcName = `_flow_studio_${this.sanitizeFunctionName(cmd.name)}`;
    const lines: string[] = [];
    ...
    if (cmd.subcommands && cmd.subcommands.length > 0) {
      ...
      for (const subcmd of cmd.subcommands) {
        ...
        lines.push(`          _flow_studio_${this.sanitizeFunctionName(cmd.name)}_${this.sanitizeFunctionName(subcmd.name)}`);
        lines.push('          ;;');
      }
  ...
  private generateSubcommandFunction(parentName: string, subcmd: CommandDefinition): string[] {
    const funcName = `_flow_studio_${this.sanitizeFunctionName(parentName)}_${this.sanitizeFunctionName(subcmd.name)}`;
    const lines: string[] = [];
  ...
  private generatePositionalSpec(positionalType?: string): string {
    switch (positionalType) {
      case 'change-id':
        return "'*: :_flow_studio_complete_changes'";
      case 'spec-id':
        return "'*: :_flow_studio_complete_specs'";
      case 'change-or-spec-id':
        return "'*: :_flow_studio_complete_items'";
      case 'schema-name':
        return "'*: :_flow_studio_complete_schemas'";
      case 'path':
        return "'*:path:_files'";
      case 'shell':
  ...
  private generateIndexedPositionalSpec(
    ...
    switch (positional.type) {
      case 'change-id':
        return `'${index}${separator}${name}:_flow_studio_complete_changes'`;
      case 'spec-id':
        return `'${index}${separator}${name}:_flow_studio_complete_specs'`;
      case 'change-or-spec-id':
        return `'${index}${separator}${name}:_flow_studio_complete_items'`;
      case 'schema-name':
        return `'${index}${separator}${name}:_flow_studio_complete_schemas'`;
      case 'path':

...
# Use flow-studio __complete to get available changes
_flow_studio_complete_changes() {
  local -a changes
  while IFS=$'\\t' read -r id desc; do
    changes+=("$id:$desc")
  done < <(flow-studio __complete changes 2>/dev/null)
  _describe "change" changes
}

# Use flow-studio __complete to get available specs
_flow_studio_complete_specs() {
  local -a specs
  while IFS=$'\\t' read -r id desc; do
    specs+=("$id:$desc")
  done < <(flow-studio __complete specs 2>/dev/null)
...
# Use flow-studio __complete to get available schemas
_flow_studio_complete_schemas() {
  local -a schemas
  while IFS=$'\\t' read -r id desc; do
    schemas+=("$id:$desc")
  done < <(flow-studio __complete schemas 2>/dev/null)

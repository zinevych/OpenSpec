...
const METADATA_FILENAME = '.flow-studio.yaml';
...
/**
 * Writes change metadata to .flow-studio.yaml in the change directory.
 *
...
/**
 * Reads change metadata from .flow-studio.yaml in the change directory.
 *
 * @param changeDir - The path to the change directory
 * @param projectRoot - Optional project root for project-local schema resolution
 ...
 * 2. Schema from .flow-studio.yaml metadata (if exists)
 * 3. Schema from flow-studio/config.yaml (if exists)
...
export function resolveSchemaForChange(
  ...
  // Derive project root from changeDir (changeDir is typically projectRoot/flow-studio/changes/change-name)

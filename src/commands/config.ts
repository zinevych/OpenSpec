...
import { FLOW_STUDIO_DIR_NAME } from '../core/config.js';
...
const WORKFLOW_PROMPT_META: Record<string, WorkflowPromptMeta> = {
  ...
  onboard: {
    name: 'Onboard',
    description: 'Guided onboarding flow for Flow Studio',
  },
};
...
function maybeWarnConfigDrift(
  ...
  const openspecDir = path.join(projectDir, FLOW_STUDIO_DIR_NAME);
  ...
  console.log(colorize('Warning: Global config is not applied to this project. Run `flow-studio update` to sync.'));
}
...
export function registerConfigCommand(program: Command): void {
  ...
  configCmd
    ...
    .action((key: string, value: string, options: { string?: boolean; allowUnknown?: boolean }) => {
      ...
      if (!keyValidation.valid && !allowUnknown) {
        ...
        console.error('Use "flow-studio config list" to see available keys.');
  ...
  configCmd
    ...
    .action(async (options: { all?: boolean; yes?: boolean }) => {
      if (!options.all) {
        ...
        console.error('Usage: flow-studio config reset --all [-y]');
        process.exitCode = 1;
        return;
      }
  ...
  configCmd
    ...
    .action(async (preset?: string) => {
      // Preset shortcut: `flow-studio config profile core`
      if (preset === 'core') {
        const config = getGlobalConfig();
        config.profile = 'core';
        ...
        console.log('Config updated. Run `flow-studio update` in your projects to apply.');
        return;
      }
      ...
      if (!process.stdout.isTTY) {
        console.error('Interactive mode required. Use `flow-studio config profile core` or set config via environment/flags.');
        process.exitCode = 1;
        return;
      }
      ...
      try {
        ...
        // Check if inside an Flow Studio project
        ...
        if (fs.existsSync(openspecDir)) {
          ...
          if (applyNow) {
            try {
              execSync('npx flow-studio update', { stdio: 'inherit', cwd: projectDir });
              console.log('Run `flow-studio update` in your other projects to apply.');
            } catch {
              console.error('`flow-studio update` failed. Please run it manually to apply the profile changes.');
              process.exitCode = 1;
            }
            return;
          }
